...
#  And check that our mean is zero and standard deviation is zero for each
...
tab <- table(iris_test_predictions,iris_test_response)
tab
...
for (i in seq_along(k_to_try)) {
  pred = knn(train = iris_train, 
             test  = iris_test, 
             cl    = iris_test_predictions, 
             k     = k_to_try[i])
  err_k[i] = calc_class_err(iris_test_predictions, pred)
}
...
ggplot(explore_k, aes(x = k, y = error_val)) +
  ...
  geom_hline(h = min(error_val), color = "darkorange") +
